...
# added by elsigh because we both have "models" modules.
mymodelspath =  re.sub('/gaebar/views.py[c]?', '', __file__) + '/models'
sys.path.append(mymodelspath)

for modelstuple in settings.GAEBAR_MODELS:
  model_package = modelstuple[0]
  model_classes = modelstuple[1]
  #logging.info('model_package: %s, model_classes: %s' % (model_package, model_classes))
